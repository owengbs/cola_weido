...
class WikiParser(Parser):
    ...
    def store(self, url, title, content, category, last_update):
        try:
            ...
            pattern = re.compile(r'.*计算机视觉.*')
            match = pattern.match(content)
            if not match:
                return
            doc = WikiDocument(url = url, title=title, content=content, category = category, last_update=last_update)
            doc.save()
    ...
    def _extract(self, soup):
        ...
        catlinks = soup.find('div', attrs={'id': 'catlinks', 'class': 'catlinks'}).find('ul').findAll('li')
        catlinks = [tagi.text for tagi in catlinks]
        catlinks = '-'.join(catlinks)
        ...
        return title, content, catlinks, last_update
    ...
    def parse(self, url=None):
        ...
        title, content, category, last_update = self._extract(soup)
        if title is None:
            return []
        title = title + ' ' + lang
        self.store(url, title, content, category, last_update)
